#!/usr/bin/env bash
# Compose 3 PNG cuts into a 15s seamless condo TV loop MP4
#
# Usage:
#   bash scripts/animate-condo-loop.sh
#   # or: make animate-loop
#
# Requires:
#   - ffmpeg
#   - ads/condo-tv-loop-15s/clarity-condo-cut{1,2,3}.png

set -euo pipefail

CUTS_DIR="ads/condo-tv-loop-15s"
OUTPUT="$CUTS_DIR/clarity-condo-loop-15s.mp4"

# ─── Check inputs ──────────────────────────────────────────
for i in 1 2 3; do
  if [[ ! -f "$CUTS_DIR/clarity-condo-cut${i}.png" ]]; then
    echo "✗ Missing: $CUTS_DIR/clarity-condo-cut${i}.png"
    echo "→ Run: make generate-t2i  or  make generate-i2i"
    exit 1
  fi
done
...
echo "Composing 3 cuts → 15s loop..."
...
ffmpeg -y -loglevel error -stats \
  ...
  -filter_complex "
    [0:v]format=yuv420p,fps=30,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black[v0];
    [1:v]format=yuv420p,fps=30,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black[v1];
    [2:v]format=yuv420p,fps=30,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black[v2];
